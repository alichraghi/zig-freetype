...
fn buildFreeType(b: *std.build.Builder, mode: std.builtin.Mode, target: std.zig.CrossTarget, custom_config_path: ?[]const u8, comptime options: Options) !*std.build.LibExeObjStep {
    ...
    if (options.cff) {
        if (!options.sfnt) {
            @compileError("CFF requires SFNT module");
        }
        if (!options.pshinter) {
            @compileError("CFF requires PSHinter module");
        }
        if (!options.psnames) {
            @compileError("CFF requires PSNames module");
        }
    ...
    if (options.type1cid) {
        if (!options.psaux) {
            @compileError("Type1CID requires PSAux module");
        }
        if (!options.pshinter) {
            @compileError("Type1CID requires PSHinter module");
        }
        if (!options.psnames) {
            @compileError("Type1CID requires PSNames module");
        }
    ...
    if (options.pcf) {
        try sources.append(ft_root ++ "/src/pcf/pcf.c");
    }
    ...
    if (options.sfnt) {
        try sources.append(ft_root ++ "/src/sfnt/sfnt.c");
    }

    if (options.truetype) {
        if (!options.sfnt) {
            @compileError("TrueType requires SFNT module");
        }
        if (!options.psnames) {
            @compileError("TrueType requires PSNames module");
        }
    ...
    if (options.type1) {
        if (!options.psaux) {
            @compileError("Type1 requires PSAux module");
        }
        if (!options.pshinter) {
            @compileError("Type1 requires PSHinter module");
        }
        if (!options.psnames) {
            @compileError("Type1 requires PSNames module");
        }
    ...
    if (options.type42) {
        if (!options.truetype) {
            @compileError("Type42 requires TrueType module");
        }
    ...
    if (options.autofit) {
        try sources.append(ft_root ++ "/src/autofit/autofit.c");
    }

    if (options.cache_subsystem) {
        try sources.append(ft_root ++ "/src/cache/ftcache.c");
    }

    if (options.gzip) {
        try sources.append(ft_root ++ "/src/gzip/ftgzip.c");
    }

    if (options.lzw) {
        try sources.append(ft_root ++ "/src/lzw/ftlzw.c");
    }

    if (options.bzip2) {
        try sources.append(ft_root ++ "/src/bzip2/ftbzip2.c");
    }

    if (options.gx_validation) {
        try sources.append(ft_root ++ "/src/gxvalid/gxvalid.c");
    }

    if (options.opentype_validation) {
        try sources.append(ft_root ++ "/src/otvalid/otvalid.c");
    }

    if (options.psaux) {
        try sources.append(ft_root ++ "/src/psaux/psaux.c");
    }

    if (options.pshinter) {
        try sources.append(ft_root ++ "/src/pshinter/pshinter.c");
    }

    if (options.psnames) {
        try sources.append(ft_root ++ "/src/psnames/psnames.c");
    }

    if (options.svg) {
        try sources.append(ft_root ++ "/src/svg/ftsvg.c");
    }

    if (options.raster) {
        try sources.append(ft_root ++ "/src/raster/raster.c");
    }

    if (options.sdf) {
        try sources.append(ft_root ++ "/src/sdf/sdf.c");
    }

    if (options.smooth) {
        try sources.append(ft_root ++ "/src/smooth/smooth.c");
    }
